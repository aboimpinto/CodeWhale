...
//! the model — only the body after the closing `---` is used.
//! Supported frontmatter fields: `description`, `allowed-tools`, `argument-hint`.
...
/// the frontmatter is stripped — only the body after the closing `---`
...
mod tests {
    ...
    fn test_frontmatter_is_stripped_from_dispatch() {
        ...
        let tmp = TempDir::new().unwrap();
        let ws = tmp.path().to_path_buf();
        // Command with frontmatter — the body is just "Run tests"
        write_command(
            &ws.join(".deepseek").join("commands"),
            "test-runner",
    ...
    fn test_workspace_local_shadows_global_by_name() {
        ...
        // Claude dir version — should be shadowed by .deepseek
        write_command(
            &ws.join(".claude").join("commands"),
            "shared",
            "claude version",
