...
//!
//! Users can drop self-describing scripts in `~/.deepseek/tools/` and they
    ...
    async fn execute(&self, input: Value, _context: &ToolContext) -> Result<ToolResult, ToolError> {
        ...
        if let Some(ref mut stdin) = child.stdin {
            stdin
                ...
                .await
                .map_err(|e| ToolError::execution_failed(format!("failed to write stdin: {e}")))?;
            stdin
                .shutdown()
                .await
                .map_err(|e| ToolError::execution_failed(format!("failed to close stdin: {e}")))?;
        }
        ...
        if output.status.success() {
            ...
            // Try to parse as ToolResult JSON, fall back to wrapping raw output.
