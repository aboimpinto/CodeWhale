...
mod tests {
    ...
    #[test]
    fn python_constants() {
        assert_eq!(Python::runtime_name(), "Python");
        assert_eq!(Python::file_extension(), "py");
        assert_eq!(Python::tool_name(), "code_execution");
    }

    #[test]
    fn python_tool_definition_has_correct_structure() {
        let tool = Python::tool_definition();
        assert_eq!(tool.name, "code_execution");
        assert!(tool.description.contains("Python"));
        assert_eq!(tool.tool_type.as_deref(), Some("code_execution_20250825"));
    ...
    #[test]
    fn python_prepare_command_adds_script_path() {
        let mut cmd = tokio::process::Command::new("python");
        let script = Path::new("/tmp/test.py");
        <Python as RuntimeTool>::prepare_command(&mut cmd, script, Path::new("/tmp"));
        let args: Vec<_> = cmd.as_std().get_args().collect();
        assert_eq!(args, vec![std::ffi::OsStr::new("/tmp/test.py")]);
    }
    ...
    #[test]
    fn node_constants() {
        assert_eq!(Node::runtime_name(), "Node.js");
        assert_eq!(Node::file_extension(), "js");
        assert_eq!(Node::tool_name(), "js_execution");
    }

    #[test]
    fn node_tool_definition_has_correct_structure() {
        let tool = Node::tool_definition();
        assert_eq!(tool.name, "js_execution");
        assert!(tool.description.contains("JavaScript"));
        assert!(tool.description.contains("Node.js"));
        assert_eq!(tool.tool_type.as_deref(), Some("code_execution_20250825"));
    }

    #[test]
    fn node_prepare_command_adds_script_path() {
        let mut cmd = tokio::process::Command::new("node");
        let script = Path::new("/tmp/test.js");
        <Node as RuntimeTool>::prepare_command(&mut cmd, script, Path::new("/tmp"));
        let args: Vec<_> = cmd.as_std().get_args().collect();
        assert_eq!(args, vec![std::ffi::OsStr::new("/tmp/test.js")]);
    }
    ...
    #[test]
    fn dotnet_constants() {
        assert_eq!(DotNet::runtime_name(), ".NET");
        assert_eq!(DotNet::file_extension(), "cs");
        assert_eq!(DotNet::tool_name(), "dotnet_execution");
    }

    #[test]
    fn dotnet_tool_definition_has_correct_structure() {
        let tool = DotNet::tool_definition();
        assert_eq!(tool.name, "dotnet_execution");
        assert!(tool.description.contains("C#"));
        assert!(tool.description.contains(".NET"));
        assert_eq!(tool.tool_type.as_deref(), Some("code_execution_20250825"));
    }

    #[test]
    fn dotnet_prepare_command_adds_run_before_script_path() {
        let mut cmd = tokio::process::Command::new("dotnet");
        let script = Path::new("/tmp/test.cs");
        <DotNet as RuntimeTool>::prepare_command(&mut cmd, script, Path::new("/tmp"));
        let args: Vec<_> = cmd.as_std().get_args().collect();
        assert_eq!(
            args,
            vec![
                std::ffi::OsStr::new("run"),
                std::ffi::OsStr::new("/tmp/test.cs"),
            ]
        );
    }
    ...
    #[test]
    fn go_constants() {
        assert_eq!(Go::runtime_name(), "Go");
        assert_eq!(Go::file_extension(), "go");
        assert_eq!(Go::tool_name(), "go_execution");
    }

    #[test]
    fn go_tool_definition_has_correct_structure() {
        let tool = Go::tool_definition();
        assert_eq!(tool.name, "go_execution");
        assert!(tool.description.contains("Go"));
        assert_eq!(tool.tool_type.as_deref(), Some("code_execution_20250825"));
    }

    #[test]
    fn go_prepare_command_adds_run_before_script_path() {
        let mut cmd = tokio::process::Command::new("go");
        let script = Path::new("/tmp/test.go");
        <Go as RuntimeTool>::prepare_command(&mut cmd, script, Path::new("/tmp"));
        let args: Vec<_> = cmd.as_std().get_args().collect();
        assert_eq!(
            args,
            vec![
                std::ffi::OsStr::new("run"),
                std::ffi::OsStr::new("/tmp/test.go"),
            ]
        );
    }
    ...
    #[test]
    fn typescript_constants() {
        assert_eq!(TypeScript::runtime_name(), "TypeScript");
        assert_eq!(TypeScript::file_extension(), "ts");
        assert_eq!(TypeScript::tool_name(), "ts_execution");
    }

    #[test]
    fn typescript_tool_definition_has_correct_structure() {
        let tool = TypeScript::tool_definition();
        assert_eq!(tool.name, "ts_execution");
        assert!(tool.description.contains("TypeScript"));
        assert!(tool.description.contains("ts-node"));
        assert_eq!(tool.tool_type.as_deref(), Some("code_execution_20250825"));
    }
    ...
    #[test]
    fn rust_constants() {
        assert_eq!(RustC::runtime_name(), "Rust");
        assert_eq!(RustC::file_extension(), "rs");
        assert_eq!(RustC::tool_name(), "rust_execution");
    }

    #[test]
    fn rust_tool_definition_has_correct_structure() {
        let tool = RustC::tool_definition();
        assert_eq!(tool.name, "rust_execution");
    ...
    #[test]
    fn ensure_runtime_tool_does_not_add_duplicate() {
        let mut catalog = vec![Python::tool_definition()];
        let before = catalog.len();
        ensure_runtime_tool::<Python>(&mut catalog);
        assert_eq!(catalog.len(), before, "should not add a duplicate");
    }

    #[test]
    fn ensure_runtime_tool_only_adds_when_tool_name_is_new() {
        let mut catalog: Vec<Tool> = Vec::new();
        ensure_runtime_tool::<Node>(&mut catalog);
        // Node might or might not be available depending on test env.
