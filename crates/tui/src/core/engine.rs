...
pub struct EngineConfig {
    ...
    /// Tool restriction: when set, only these tools are allowed.
    pub allowed_tools: Option<Vec<String>>,
    /// Hook executor for control-plane hooks (PreToolUse / PostToolUse).
    /// When set, the turn loop runs `ToolCallBefore` hooks before each
    /// tool execution; exit code 2 from a hook denies the tool.
...
impl Engine {
    ...
    async fn handle_send_message(
        ...
        self.config.allowed_tools = allowed_tools.clone();
        self.paused = false;
